...
public sealed class WgcScreenRecorder : IScreenRecorder
{
    ...
    private long _bufferedBytes;                // decoded size of _frames, guarded by _gate
    ...
    private void OnFrameArrived(Direct3D11CaptureFramePool sender, object args)
    {
        ...
        try
        {
            ...
            budgetExhausted = !BufferFrame(frame);
        }
        catch (Exception ex)
        {
        ...
        // MUST be raised outside _gate: the app marshals AutoStopped onto the UI thread with
        // a blocking Dispatcher.Invoke, and that handler calls Stop(), which waits for _gate.
        // Raising it while this pool thread still holds _gate deadlocks the recording.
        if (budgetExhausted)
        {
            Debug.WriteLine("WGC: frame budget exhausted — stopping the recording early.");
            RaiseAutoStop();
        }
    }

    /// <summary>
    /// Copies one frame into the buffer. Returns false when the memory budget is exhausted
    /// (nothing was buffered and the recording has to end).
    ///
    /// Holds _gate across the whole copy: it uses _vdevice/_vcontext, which Stop/Cancel
    ...
    /// </summary>
    private bool BufferFrame(Direct3D11CaptureFrame frame)
    {
        lock (_gate)
        {
            if (_disposed) return true;                       // recorder torn down — drop this frame
            double t = _clock.Elapsed.TotalSeconds;
            if (t - _lastKeptSec < 1.0 / MaxCaptureFps) return true;  // above the 10 fps grid — drop
            var bmp = CopyToBitmap(frame, _crop);
            if (bmp is null) return true;                     // V5: unreadable surface -> skip

            if (!RecordingBudget.Fits(_bufferedBytes, _frames.Count, bmp.Width, bmp.Height))
            {
                bmp.Dispose();
                return false;
            }
            _frames.Add(new RecordedFrame(bmp, t));
            _bufferedBytes += RecordingBudget.FrameBytes(bmp.Width, bmp.Height);
            _lastKeptSec = t;
            return true;
        }
    }
    ...
    /// <summary>V1: stop the timer BEFORE raising, and raise exactly once — the 60 s cap and
    /// the memory ceiling can both trip, and the handler tears the whole recorder down.</summary>
    private void RaiseAutoStop()
    {
        if (System.Threading.Interlocked.Exchange(ref _autoStopRaisedFlag, 1) != 0) return;
    ...
    {
        // null adapter = default; null feature levels = the D3D11 default chain. The
        // bindings type both as non-nullable, hence the explicit null-forgiving casts.
        var noFeatureLevels = (FeatureLevel[]?)null!;
        var res = D3D11.D3D11CreateDevice(
            null, DriverType.Hardware, DeviceCreationFlags.BgraSupport, noFeatureLevels,
            out ID3D11Device? device, out ID3D11DeviceContext? context);
        if (res.Failure || device is null || context is null)
        {
            // Retry on WARP (headless / RDP / no GPU). Release whatever the first
            // attempt handed back so a half-created device isn't leaked.
            device?.Dispose(); context?.Dispose();
            res = D3D11.D3D11CreateDevice(
                null, DriverType.Warp, DeviceCreationFlags.BgraSupport, noFeatureLevels,
    ...
    private static System.Drawing.Bitmap BuildBitmap(
        nint src, int srcStride, int fullW, int fullH, PixelRect? crop)
    {
        ...
        var full = CopyRows(src, srcStride, cx, cy, cw, ch);

        // Downscale HERE, not at render time. Recorded frames only ever feed the GIF
        // pipeline, which caps at RecordingBudget.MaxCaptureWidth anyway — so keeping the
        // native resolution buffered only bought an out-of-memory crash (~18.5 GiB for a
        // 60 s 4K recording, over 93 % of it thrown away again in GifRenderer.Scale).
        var (tw, th) = GifPlan.ScaledSize(cw, ch, RecordingBudget.MaxCaptureWidth);
        if (tw == cw && th == ch) return full;
        using (full) return Downscale(full, tw, th);
    }

    private static System.Drawing.Bitmap Downscale(System.Drawing.Bitmap src, int w, int h)
    {
        var dst = new System.Drawing.Bitmap(w, h, PixelFormat.Format32bppArgb);
    ...
    /// <summary>Copies the (cropped) mapped surface rows into a managed bitmap, undoing
    /// WGC's premultiplied alpha on the way.</summary>
    private static unsafe System.Drawing.Bitmap CopyRows(
        nint src, int srcStride, int cx, int cy, int cw, int ch)
    {
        ...
        try
        {
            ...
            for (int row = 0; row < ch; row++)
            {
                ...
                // WGC surfaces are premultiplied BGRA; α < 255 under menu shadows /
                // acrylic would otherwise bake dark "shadow rectangles" into GIFs.
                CaptureAlpha.UnpremultiplyRowToOpaque(new Span<byte>(dstRow, rowBytes));
            }
        }
        finally
        {
