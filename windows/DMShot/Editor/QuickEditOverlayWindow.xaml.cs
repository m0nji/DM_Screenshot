...
public partial class QuickEditOverlayWindow : Window
{
    ...
    // Full-bleed inside the overlay's own accent-framed panel: no CornerRadius (mac passes
    // pad: 0 / cornerRadius 0 here too). The work-area brush still follows the design.
    ...
    private Border BuildToolbar()
    {
        ...
        foreach (var (kind, geo, fill) in QuickTools)
        {
            var tb = new RadioButton
            {
                ...
                Content = VectorIcon(geo, fill), ToolTip = Loc.Instance[ToolTipKey(kind)],
    ...
    /// (so it turns dark when the chip is the active/accent tool). Named VectorIcon, not
    /// Icon: the latter shadows Window.Icon (CS0108) on this Window subclass.</summary>
    private static UIElement VectorIcon(string data, bool fill)
    {
