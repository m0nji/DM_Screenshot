...
public static class GifEncoder
{
    ...
    /// </summary>
    /// <param name="disposeInputs">
    /// Dispose each input bitmap as soon as its pixels have been copied into the ImageSharp
    /// image. ImageSharp keeps its own full copy of every frame, so leaving the inputs alive
    /// doubles the peak: measured 3298 MiB for 500 frames of 1000x562, of which 1071 MiB were
    /// the inputs just sitting there. Only pass true when the caller is done with them.
    /// </param>
    public static byte[] EncodeWithDelays(IReadOnlyList<Drawing.Bitmap> frames, IReadOnlyList<double> delaysSec,
                                          bool disposeInputs = false)
    {
        ...
        if (disposeInputs) frames[0].Dispose();
        ...
        for (int i = 1; i < frames.Count; i++)
        {
            ...
            if (disposeInputs) frames[i].Dispose();
        }
        ...
        gif.SaveAsGif(ms, new IsGifEncoder
        {
            ...
            // MaxColors 255, NOT the default 256: frames 2..n are delta-encoded, which needs
            // a free palette slot for the transparent index ("unchanged — show the previous
            // frame"). With all 256 slots spent on real colours the encoder has to reuse a
            // colour entry for transparency, and every unchanged pixel inside the delta
            // rectangle then PAINTS that colour instead of letting the previous frame through.
            // On a desktop recording that entry is near-black, so the moving window came out
            // as a black rectangle with speckles where pixels really changed, while static
            // areas outside the delta rectangle stayed correct. Costs one palette colour.
            Quantizer      = new WuQuantizer(new QuantizerOptions { Dither = null, MaxColors = 255 }),
        });
        return ms.ToArray();
    }
    ...
    {
        // The renderer borrows frames instead of copying them, so the SAME bitmap can be
        // sampled for two consecutive grid times. LockBits twice on one bitmap throws
        // ("Bitmap region is already locked") — and a bitmap trivially does not differ
        // from itself.
        if (ReferenceEquals(a, b)) return 0.0;
    ...
    private static Image<Bgra32> ToImageSharp(Drawing.Bitmap bmp)
    {
        ...
        try
        {
            ...
            // A GIF frame is opaque by definition. Junk alpha from GDI/WGC captures must
            // never reach the quantizer: its palette distance includes α, and low-α pixels
            // risk the transparent index (→ ghost holes) on other ImageSharp versions.
