...
public partial class VideoPreviewWindow : Window, IDisposable
{
    ...
    /// <summary>Undo <see cref="Raise"/> after a failed render: the frames are still ours,
    /// so hand the window back to the user (playback, trim, retry) instead of leaving it
    /// frozen on "Creating GIF…" or throwing the recording away.</summary>
    public void ResetAfterFailedRender()
    {
        if (_disposed) return;
        _createdGif = false;              // a close from here must dispose the frames again
        _rendering = false;
        Cursor = null;
        RenderingLabel.Visibility = Visibility.Collapsed;
        DurationLabel.Visibility = Visibility.Visible;
        EstimatedSizeLabel.Visibility = Visibility.Visible;
        DiscardButton.IsEnabled = true;
        QualityStandard.IsEnabled = true;
        QualitySmall.IsEnabled = true;
        UpdateCreateGifEnabled();
        _timer.Start();
    }
