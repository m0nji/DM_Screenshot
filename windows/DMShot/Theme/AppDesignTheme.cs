...
public static class AppDesignTheme
{
    ...
    public static void Apply(ResourceDictionary resources, AppDesign design)
    {
        ...
        // DmBackground is the brand background token (--dm-bg) and stays the caption colour
        // for gradient-backed windows. It is NOT the editor chrome: at #090908 under a
        // #181614 toolbar it reads as a black title bar band (mac Theme.swift parity).
        SetBrush(resources, "DmBackground", pick("#090908", "#000000", "#1F1F1F"));
        SetBrush(resources, "DmSurface", pick("#181614", "#060606", "#212121"));
        // Work area == chrome; the hatch below, not a tone, is what tells an empty
        // canvas from a screenshot that merely happens to be dark (mac Theme.swift).
        SetBrush(resources, "DmCanvas", pick("#181614", "#060606", "#212121"));
        SetCanvasHatch(resources, pick("#342F2A", "#222226", "#343438"));
        ...
        SetBrush(resources, "DmRowHover", pick("#F5F1EA", "#E6E6EA", "#DEDEE2"), 0.07);
    ...
    // Diagonal hatch over the empty work area (the Shottr cue), as a tiling brush: one
    // fill per frame instead of a few hundred strokes in the canvas draw path.
    // Period 10 DIP, matching Theme.canvasHatchPeriod on macOS.
    private static void SetCanvasHatch(ResourceDictionary resources, string lineHex)
    {
        const double period = 10;
        if (ColorConverter.ConvertFromString(lineHex) is not Color color) return;

        var pen = new Pen(new SolidColorBrush(color), 1);
        // One diagonal per tile: repeated, it joins corner to corner into an unbroken
        // family of 45° stripes.
        var line = new GeometryDrawing(
            null, pen, new LineGeometry(new Point(0, 0), new Point(period, period)));
        var hatch = new DrawingBrush(line)
        {
            TileMode = TileMode.Tile,
            Viewport = new Rect(0, 0, period, period),
            ViewportUnits = BrushMappingMode.Absolute,
            Stretch = Stretch.None,
        };
        if (hatch.CanFreeze) hatch.Freeze();
        resources["DmCanvasHatch"] = hatch;
    }
