...
public class BlackUtilityThemeTests
{
    ...
    public void WindowsThemeUsesBlackUtilityPalette()
    {
        ...
        Assert.Contains("x:Key=\"DmCanvas\" Color=\"#000000\"", theme);
    ...
    public void WindowsControlsUseSoftenedChromeBaseBorder()
    {
        ...
        // Used broadly, not just on one control. The Graphite Sand pass (0.9.0) folded the
        // settings nav rows into one grouped surface, which dropped a resting border — hence
        // 7 rather than the 8 this expected before.
        var uses = theme.Split("{DynamicResource DmBorderControlSoft}").Length - 1;
        Assert.True(uses >= 7, $"expected the softened base border on most chromed controls, found {uses}");
    }
    ...
    public void WindowsSettingsWindowUsesThemedBlackSurfaces()
    {
        ...
        // Window chrome must come from the design system (AppDesignTheme swaps it per design),
        // never a hardcoded color. Since 0.9.0 the nav and the content pane are *grouped
        // surfaces* — a transparent panel whose own DmBorder frame separates it — instead of
        // a filled DmNavBackground sidebar, so the window is one calm tone (macOS parity).
        Assert.Contains("Background=\"{DynamicResource DmWindowBackground}\"", settings);
        Assert.Contains("BorderBrush=\"{DynamicResource DmBorder}\"", settings);
        // Two grouped surfaces: the nav and the content pane.
        Assert.Equal(2, settings.Split("BorderBrush=\"{DynamicResource DmBorder}\"").Length - 1);
    ...
    public void WindowsEditorCanvasUsesSelectedDesignSurface()
    {
        ...
        // Chrome is one surface up to the top edge; only the work area steps back, as an
        // inset card (macOS EditorView parity).
        // No left margin: the 8 px GridSplitter beside the card already provides that gap,
        // so an extra one would double it (0.9.0 "even spacing on all sides").
        Assert.Contains("Grid.Column=\"2\" Margin=\"0,8,8,8\" CornerRadius=\"12\"", editor);
        Assert.Contains("Background=\"{DynamicResource DmCanvas}\"", editor);
        Assert.Contains("CanvasBackground=\"{DynamicResource DmCanvas}\"", editor);
        Assert.Contains("CanvasHatch=\"{DynamicResource DmCanvasHatch}\"", editor);
        Assert.DoesNotContain("Background=\"{DynamicResource DmBackground}\"", editor);
        ...
        Assert.DoesNotContain("BorderThickness=\"0,0,0,1\"", editor);
        Assert.DoesNotContain("BorderThickness=\"0,0,1,0\"", editor);
        Assert.DoesNotContain("Background=\"{DynamicResource DmBorder}\" Cursor=\"SizeWE\"", editor);
        Assert.Contains("<Trigger Property=\"IsMouseOver\" Value=\"True\">", editor);

        // The canvas brush used to be a hardcoded #141418 static that ignored the design.
        var canvas = Read("windows/DMShot/Editor/CanvasControl.cs");
        Assert.Contains("CanvasBackgroundProperty", canvas);
        Assert.Contains("dc.DrawRectangle(CanvasBackground, null", canvas);
        Assert.DoesNotContain("MakeFrozen(Color.FromRgb(0x14, 0x14, 0x18))", canvas);
    }

    [Fact]
    public void WindowsEditorCaptionBlendsIntoTheChrome()
    {
        var titleBar = Read("windows/DMShot/Platform/DarkTitleBar.cs");
        var editor = Read("windows/DMShot/Editor/EditorWindow.xaml.cs");

        // titlebarAppearsTransparent parity: the caption has to match whatever the window's
        // own backdrop starts with, or it reads as a black band above the toolbar.
        Assert.Contains("enum CaptionBackdrop", titleBar);
        Assert.Contains("\"DmSurface\" : \"DmBackground\"", titleBar);
        Assert.Contains("DarkTitleBar.SetBackdrop(this, DarkTitleBar.CaptionBackdrop.Chrome)", editor);
    }
    ...
    public void WindowsSettingsExposeMacDesignSwitcherAndToggleParity()
    {
        ...
        Assert.Contains("public enum AppDesign { Standard, Black, GraphiteSand }", settingsModel);
        Assert.Contains("public AppDesign AppDesign { get; set; } = AppDesign.GraphiteSand;", settingsModel);
        Assert.Contains("ShowDesignPicker()", settingsWindow);
        Assert.Contains("Loc.Instance[\"design\"]", settingsWindow);
        Assert.Contains("AppDesign.GraphiteSand", settingsWindow);
        ...
        Assert.Contains("[\"designGraphiteSand\"] = \"Graphite Sand\"", loc);
        Assert.Contains("[\"designStandard\"] = \"Standard Design\"", loc);
        Assert.Contains("[\"designBlack\"] = \"Black Utility\"", loc);
    }

    [Fact]
    public void GraphiteSandPaletteMatchesBrandTokens()
    {
        // Values from DM_CICD 03_colors_tokens/dm-apps-brand-tokens.css (dark theme).
        var resources = new ResourceDictionary();
        AppDesignTheme.Apply(resources, AppDesign.GraphiteSand);

        Assert.Equal(Color.FromRgb(0x09, 0x09, 0x08), ((SolidColorBrush)resources["DmBackground"]).Color);   // --dm-bg
        Assert.Equal(Color.FromRgb(0x18, 0x16, 0x14), ((SolidColorBrush)resources["DmSurface"]).Color);      // --dm-surface
        // Work area == chrome; the hatch, not a fill, marks it as empty.
        Assert.Equal(Color.FromRgb(0x18, 0x16, 0x14), ((SolidColorBrush)resources["DmCanvas"]).Color);
        Assert.IsType<DrawingBrush>(resources["DmCanvasHatch"]);
        Assert.Equal(Color.FromRgb(0x23, 0x20, 0x1D), ((SolidColorBrush)resources["DmSurfaceLight"]).Color); // --dm-surface-2
        Assert.Equal(Color.FromRgb(0xC7, 0xB2, 0x99), ((SolidColorBrush)resources["DmAccent"]).Color);       // --dm-primary
        Assert.Equal(Color.FromRgb(0xC7, 0xB2, 0x99), ((SolidColorBrush)resources["DmAccentFill"]).Color);   // light fill + dark label (Workspace parity)
        Assert.Equal(Color.FromRgb(0xC7, 0xB2, 0x99), ((SolidColorBrush)resources["DmSwitchOn"]).Color);
        Assert.Equal(1, ((SolidColorBrush)resources["DmSwitchOn"]).Opacity);
        Assert.Equal(Color.FromRgb(0x17, 0x15, 0x12), ((SolidColorBrush)resources["DmOnAccent"]).Color);     // dark label on the sand fill
        Assert.Equal(Color.FromRgb(0xF5, 0xF1, 0xEA), ((SolidColorBrush)resources["DmText"]).Color);         // --dm-text
        Assert.Equal(Color.FromRgb(0xA9, 0xA3, 0x9A), ((SolidColorBrush)resources["DmTextDim"]).Color);      // --dm-muted
        Assert.Equal(Color.FromRgb(0x34, 0x2F, 0x2A), ((SolidColorBrush)resources["DmBorder"]).Color);       // --dm-line
    }

    [Fact]
    public void OtherDesignsKeepTheOrangeFamilyAccent()
    {
        foreach (var design in new[] { AppDesign.Standard, AppDesign.Black })
        {
            var resources = new ResourceDictionary();
            AppDesignTheme.Apply(resources, design);
            Assert.Equal(Color.FromRgb(0xC9, 0x7B, 0x4A), ((SolidColorBrush)resources["DmAccent"]).Color);
            Assert.Equal(Color.FromRgb(0xC9, 0x7B, 0x4A), ((SolidColorBrush)resources["DmAccentFill"]).Color);
            Assert.Equal(Colors.White, ((SolidColorBrush)resources["DmOnAccent"]).Color);
        }
    }
