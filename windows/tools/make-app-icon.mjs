...
// SVG source used by macOS. Windows gets a full-bleed export so the icon has
// comparable visual weight in the taskbar and Explorer.
//
// Deps: rsvg-convert (librsvg) on PATH.
// Run:  node windows/tools/make-app-icon.mjs
...
const out = join(repo, 'windows', 'DMShot', 'Resources', 'AppIcon.ico');
...
const work = mkdtempSync(join(tmpdir(), 'dmscreenshot-ico-'));
try {
