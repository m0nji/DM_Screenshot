...
final class LocalizationTests: XCTestCase {
    ...
    func testGermanDiffersFromEnglishForTranslatableKeys() {
        ...
        // designGraphiteSand is a brand name and stays untranslated.
        let identical: Set<L> = [.ok, .hex, .pixelsSuffix, .toolText, .toolEllipse, .sectionUpdates, .version, .startLabel, .gifViewerTitle, .gifQualityStandard, .designGraphiteSand]
        let loc = Localizer(language: .english)
