...
final class SelectionView: NSView {
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        if let sel = selection {
            ...
            let label = SelectionReadout.label(viewSize: sel.size, scale: capture.scale)
...
final class OverlayController {
    ...
    /// True while selection overlays are on screen — one of the update prompt's
    /// quiet zones (spec 2026-08-02).
