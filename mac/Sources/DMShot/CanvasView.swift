...
final class CanvasNSView: NSView, NSTextViewDelegate {
    ...
    /// Round the work area into an inset card with a hairline edge (macOS Settings
    /// grouping, DM Apps --dm-radius-sm). Done on the layer rather than with a
    /// SwiftUI `clipShape`: a mask over the hosted AppKit view would add a
    /// compositing pass to the very draw path whose feedback loop we just closed.
    /// `cornerRadius == 0` leaves the canvas full-bleed (quick-edit overlay).
    func applyCardChrome(cornerRadius: CGFloat) {
        wantsLayer = true
        ...
        layer.borderColor = cornerRadius > 0 ? appDesign.borderNSColor.cgColor : nil
    }
    ...
    private func updateZoomIndicator(percent: Int) {
        guard ZoomIndicatorPolicy.shouldPublish(
            isKeyWindow: window?.isKeyWindow == true,
            current: model.zoomPercent, computed: percent) else { return }
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        appDesign.canvasNSColor.setFill()
        bounds.fill()
        // Hatch on top of the fill, under everything else: where no image covers it,
        // the texture says "empty" — a dark screenshot no longer looks like a void.
        appDesign.canvasHatchNSColor.setFill()
        bounds.fill()
...
struct CanvasView: NSViewRepresentable {
    ...
    var pad: CGFloat = 24
    /// Rounded, hairline-bordered work area — the editor's inset canvas card.
    /// 0 keeps the canvas full-bleed, which is what the quick-edit overlay wants
    /// inside its own floating panel.
    ...
    func makeNSView(context: Context) -> CanvasNSView {
        ...
        view.applyCardChrome(cornerRadius: cornerRadius)
    ...
    func updateNSView(_ nsView: CanvasNSView, context: Context) {
        ...
        nsView.applyCardChrome(cornerRadius: cornerRadius)
        nsView.refresh()
    }
