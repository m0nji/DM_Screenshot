...
enum Theme {
    ...
    /// Corner radius of the inset canvas card — --dm-radius-sm from the DM Apps
    /// tokens. The larger brand radii (20/28/36) belong to web cards, not to
    /// app chrome.
    static let canvasCornerRadius: CGFloat = 12

    /// Spacing of the diagonal hatch behind the work area. An empty canvas has to be
    /// tellable from a screenshot that merely happens to be dark, so the backdrop
    /// carries a texture instead of a tone of its own.
    static let canvasHatchPeriod: CGFloat = 10

    static let blackAppHex = "#000000"
    static let blackPanelHex = "#060606"
    static let blackCanvasHex = "#060606"
    ...
    static let standardCanvasHex = "#212121"
    ...
    //
    // --dm-bg is the brand background token and stays the top stop of the settings
    // gradient, but it is NOT the app's chrome: at #090908 under a #181614 toolbar
    // it read as a black titlebar band. Chrome is --dm-surface throughout; the
    // canvas sits just below it, warm rather than black.
    static let sandAppHex = "#090908"          // --dm-bg
    static let sandPanelHex = "#181614"        // --dm-surface
    static let sandCanvasHex = "#181614"       // work area == chrome; the hatch marks it, not a tone
...
extension AppDesign {
    ...
    /// Work area behind the screenshot — the chrome tone, so the canvas card shows
    /// only its frame. What separates "empty" from "a dark screenshot" is the hatch
    /// below, not a different fill.
    var canvasNSColor: NSColor {
        switch self {
        case .graphiteSand: return NSColor(hex: Theme.sandCanvasHex)
        case .standard: return NSColor(hex: Theme.standardCanvasHex)
        case .black: return NSColor(hex: Theme.blackCanvasHex)
        }
    }

    /// Diagonal hatch drawn over the empty work area (the Shottr cue). Built once per
    /// design as a tiling pattern: stroking a few hundred lines every frame would land
    /// in the very draw path whose redraw loop we just closed.
    var canvasHatchNSColor: NSColor {
        if let cached = AppDesign.hatchCache[rawValue] { return cached }
        let period = Theme.canvasHatchPeriod
        let line = borderNSColor
        let tile = NSImage(size: NSSize(width: period, height: period), flipped: false) { _ in
            line.setStroke()
            let path = NSBezierPath()
            path.lineWidth = 1
            // One diagonal per tile: repeated, it joins corner to corner into an
            // unbroken family of 45° stripes.
            path.move(to: .zero)
            path.line(to: NSPoint(x: period, y: period))
            path.stroke()
            return true
        }
        let pattern = NSColor(patternImage: tile)
        AppDesign.hatchCache[rawValue] = pattern
        return pattern
    }

    private static var hatchCache: [String: NSColor] = [:]

    var appColor: Color { Color(nsColor: appNSColor) }
    var panelColor: Color { Color(nsColor: panelNSColor) }
    var canvasColor: Color { Color(nsColor: canvasNSColor) }
    ...
    var borderNSColor: NSColor {
        switch self {
        case .graphiteSand: return NSColor(hex: Theme.sandBorderHex)
        case .standard: return NSColor(hex: Theme.standardBorderHex)
        case .black: return .dmBlackBorder
        }
    }

    var borderColor: Color { Color(nsColor: borderNSColor) }
...
/// Grouped surface — the macOS Settings pattern: ONE rounded panel that holds plain
/// rows, instead of a box around every row. Outline only: the app background runs
/// through it unchanged, so the rounded frame is the sole thing that marks the group.
struct DMGroupSurface: ViewModifier {
    let design: AppDesign
    var cornerRadius: CGFloat = Theme.canvasCornerRadius
...
extension View {
    func dmGroupSurface(_ design: AppDesign, cornerRadius: CGFloat = Theme.canvasCornerRadius) -> some View {
        modifier(DMGroupSurface(design: design, cornerRadius: cornerRadius))
    }
}

/// Plain row inside a grouped surface (macOS Settings sidebar). It owns no box: the
...
struct SidebarRowStyle: ButtonStyle {
    ...
    func makeBody(configuration: Configuration) -> some View {
        ...
        let fill: Color = active
            ? design.accentSoftColor
            : (hovered ? design.textColor.opacity(0.07) : .clear)
