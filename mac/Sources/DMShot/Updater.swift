...
final class Updater: NSObject, ObservableObject, SPUUserDriver, SPUUpdaterDelegate {
    ...
    /// Re-check cadence while the app keeps running (menu-bar apps rarely relaunch,
    /// so the launch check alone misses releases). Matches DM_Workspace's 60 min.
    /// Sparkle clamps scheduled checks to a 1 h minimum, so this is the floor.
    ...
    func start() {
        ...
        u.updateCheckInterval = Self.checkInterval  // hourly, not Sparkle's 24 h default
