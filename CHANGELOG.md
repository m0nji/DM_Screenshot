...
## 0.9.2 – 2026-08-02
- new: A waiting update now says so instead of hiding behind a dot on the tray icon — it downloads quietly in the background and then asks whether to restart, so installing it is a single click. "Later" keeps quiet for a day and then asks again, and a newer version always asks even if you postponed the previous one. Nothing interrupts a running recording, an open selection, or a full-screen presentation; the prompt waits until you are done (macOS, Windows)

## 0.9.0 – 2026-07-25
- new: The editor window is one calm surface instead of black bands and boxes — the title bar now matches the toolbar rather than sitting above it as a near-black strip, and the sidebar, the settings navigation and the settings options each became a single rounded panel holding plain entries, instead of a frame around every single button inside another frame (macOS, Windows)
- new: An empty work area is finally tellable from a dark screenshot — it carries a light diagonal hatch, so a screenshot that happens to be almost black no longer looks like nothing was loaded at all (macOS, Windows)
- fix: The Windows editor canvas follows the selected design again — it painted a fixed dark blue-grey regardless of whether Graphite Sand, Standard or Black Utility was chosen (Windows)
- fix: Spacing around the editor panels is even on all sides; the gap between the sidebar and the work area was more than three times the gap at the window edges (macOS, Windows)
